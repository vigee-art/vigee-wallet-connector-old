...
export class MyAlgoWallet implements WalletImplementation {
  ...
  async reconnect(): Promise<string[]> {
    if (await this.connect()) {
      ...
      throw new Error("error reconnecting to myalgo");
    }
  }
  getAccounts(): string[] {
    return this._accounts;
    ...
    if (this.permissionCallback) {
      return await this.permissionCallback.request({
        approved: async (): Promise<SignedTxn[]> => {
          return await this.doSign(this.getSelectedAccountAddress(), txns);
        },
  ...
  async signTeal(
    ...
    return await this.walletConn.signLogicSig(
      teal,
