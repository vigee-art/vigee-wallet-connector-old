...
export class MyAlgoWallet implements WalletImplementation {
  network: Networks;
  ...
  constructor(network: Networks, walletChoice: Wallets, defaultAccountIdx: number = 0, popupPermissionCallback: PopupPermissionCallback,) {
    this.walletChoice = walletChoice;
    this.network = network;
    this.permissionCallback = popupPermissionCallback;
  ...
  async signTeal(
    ...
    return await this.walletConn.signLogicSig(teal, this.getSelectedAccountAddress());
  }
}
