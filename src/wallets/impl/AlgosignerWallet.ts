...
export class AlgoSignerWallet implements WalletImplementation {
  constructor(
    network: Networks,
  ...
  img(inverted: boolean): string {
    return inverted ? AlgoSigner.img(logoInverted) : AlgoSigner.img(logo);
  }
  ...
  async signBytes(b: Uint8Array): Promise<Uint8Array> {
    throw new Error(
      "Method not implemented to sign bytes: " + Buffer.from(b).toString()
    );
  }

  async signTeal(teal: Uint8Array): Promise<Uint8Array> {
    throw new Error(
      "Method not implemented to decode: ." + Buffer.from(teal).toString()
    );
  }
}
